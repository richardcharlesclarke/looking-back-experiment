...
export type Answer = number | string | {missing:'prefer_not'|'cannot_assess'|'cannot_estimate'|'skipped'};
export type Question = {id:string;prompt:string;section:string;type:'continuous'|'text'|'single';max?:number;low?:string;high?:string;bands?:string[];options?:string[];target?:string;optional?:boolean;cannot?:'cannot_assess'|'cannot_estimate'};
...
export function questions(panel:Panel,role:Role,wave:Wave,speakerId?:string):Question[]{
 const p=role==='speaker'?'S2S':'S2A';
 const out:Question[]=role==='speaker'?[
 ...
 if(role==='speaker'){
  for(const speaker of panel.speakers.filter(s=>s.id!==speakerId))out.push({...scale('S2S_PREDICT_'+speaker.id,`Where do you think ${speaker.name} stands on this claim: “${panel.proposition}”`,'Understanding another view',support,10),target:speaker.id,cannot:'cannot_estimate'});
 ...
 }else if(wave==='pre')out.push(scale('S2A_FAMILIAR','How familiar are you with the subject of this panel?','Before the panel',['Not at all familiar','Slightly familiar','Somewhat familiar','Very familiar','Extremely familiar'],10),scale('S2A_IMPORTANCE','How much does this issue matter to you personally?','Before the panel',['Not at all','A little','Somewhat','A lot','Extremely'],10));
 if(wave==='post'){
  ...
  out.push(...process.map(([id,prompt])=>({...scale(p+'_POST_'+id,prompt,role==='speaker'?'The discussion':'What you heard'),cannot:'cannot_assess' as const})));
