...
process.env.STUDY_ONE_STORE_DIR=path.resolve('.local/selective-http-qa');
...
const {CONTINUOUS_INSTRUMENT_VERSION:OLD,PERSPECTIVES_INSTRUMENT_VERSION:V2,CONFLICT_INSTRUMENT_VERSION:V3,SELECTIVE_INSTRUMENT_VERSION:VERSION}=require('../.local/brufest-tests/lib/brufest/festival/scales');
const {INFORMATION_VERSION,FESTIVAL_VERSION}=require('../.local/brufest-tests/lib/brufest/festival/content');
const base='http://localhost:3141',keys=[];let cookie;
...
(async()=>{
 ...
 for(const [firstVersion,encounter] of [[undefined,'Yes'],[OLD,'Yes'],[V2,'Yes'],[V3,'Yes'],[VERSION,'Yes'],[VERSION,'No']]){
 ...
 console.log(JSON.stringify({passed:true,localOnly:true,syntheticParticipants:keys.length,checks:['legacy, continuous, perspectives, conflict and selective versions','actual HTTP storage export','exact original baselines preserved','version pinned follow-up','date gates','circle endpoints and missing','learning yes and no branches','duplicate suppression','consent and contact isolation','synthetic deletion and link revocation']}));
