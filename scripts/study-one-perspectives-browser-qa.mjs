...
export default async function perspectivesQA(page) {
  ...
  async function shot(name){
    ...
    await page.locator('.bf-question').evaluateAll(es=>Promise.all(es.flatMap(e=>e.getAnimations().map(a=>a.finished))));
  ...
  for(const wave of ['pre','post']){
    ...
    while(await page.locator('.s1-form').count()){
      ...
      for(const q of await page.locator('.bf-question').all()){
        ...
        else if(await q.locator('.s1-circles').count()){
          const slider=q.getByRole('slider');
          if((await q.locator('.s1-circle-choices [aria-pressed=true]').count())!==0)throw new Error('Circle was preselected');
          await slider.focus();if(!(await slider.getAttribute('aria-valuetext')).includes('No response'))throw new Error('Focus selected a circle');
          const choices=q.locator('.s1-circle-choices button');
          for(let i=0;i<7;i++){await choices.nth(i).click();if(Number(await slider.getAttribute('aria-valuenow'))!==i+1)throw new Error('Circle choice failed');}
          await slider.focus();await slider.press('Home');if(await slider.getAttribute('aria-valuenow')!=='1')throw new Error('Circle minimum failed');
          await slider.press('End');if(await slider.getAttribute('aria-valuenow')!=='7')throw new Error('Circle maximum failed');await slider.press('ArrowLeft');if(await slider.getAttribute('aria-valuenow')!=='6')throw new Error('Circle keyboard failed');
          await choices.nth(0).click();await slider.evaluate(e=>Promise.all(e.getAnimations({subtree:true}).map(a=>a.finished)));
          const rect=await slider.boundingBox();await page.mouse.move(rect.x+rect.width*.66,rect.y+rect.height/2);await page.mouse.down();await page.mouse.move(rect.x+rect.width*.51,rect.y+rect.height/2,{steps:8});await page.mouse.up();
          if(Number(await slider.getAttribute('aria-valuenow'))<6)throw new Error('Circle drag failed');
          const selected=await slider.getAttribute('aria-valuenow');await page.reload();await page.locator('.s1-circles').waitFor();if(await page.getByRole('slider').getAttribute('aria-valuenow')!==selected)throw new Error('Circle draft restore failed');
        }
  ...
  if(submissions.length!==2||submissions[1].answers.E1_POST_KEYNOTE_IMPACT!==9.9||submissions.some(s=>s.answers.E1_HUM_01?.missing!=='cannot_assess'||s.answers.E1_CUR_01?.missing!=='prefer_not'||s.instrumentVersion!=='brufest-study-one-perspectives-v2-2026-09-09'||s.answers.E1_HUM_02!==exactValues.E1_HUM_02))throw new Error('Submission semantics changed');
  return {prefix,reports,submissions:submissions.map(s=>({wave:s.wave,itemCount:Object.keys(s.answers).length})),checks:['all pages','branching','keyboard','validation','back retains responses','save retry','contact choice','email validation','waiting','completion','deletion','invalid link','exact continuous value without snap','versioned payload','circle selection drag keyboard restore','one question per step','no overflow']};
}
