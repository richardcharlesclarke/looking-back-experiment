...
export default function FestivalParticipant(){
  ...
  return <main className="bf-main study-one s1-participant"><header className="topbar"><Link href={STUDY_ONE_PUBLIC_PATH}>How We Disagree</Link><span>Before and after Big Brue</span></header>
    {!loaded?<section className="s1-card" role="status" aria-live="polite"><p>Opening questionnaire…</p></section>:error&&!registered&&invalidLink?<section className="s1-card"><h1 tabIndex={-1}>Personal link could not be opened</h1><p role="alert">{error}</p><p>Use the original email link or ask the research assistant to resend it. Your answers will not be matched by guessing a code.</p></section>:!view?<section className="s1-card">
      ...
      <button className="primary" disabled={busy||!agree} onClick={start}>{busy?'Opening…':'Begin before Big Brue'}</button>
...
function FestivalForm({access,wave,onSubmit}:{access:string;wave:Wave;onSubmit:(wave:Wave,answers:Answers,startedAt:string)=>Promise<void>}){
  ...
  async function next(){let invalid=current.items.find(q=>answerError(q,answers[q.id]));if(!invalid&&pageIndex===pages.length-1)invalid=items.find(q=>answerError(q,answers[q.id]));if(invalid){setInvalidId(invalid.id);setError(`${invalid.prompt} ${answerError(invalid,answers[invalid.id])}`);setPage(pages.findIndex(p=>p.items.includes(invalid!)));return;}setInvalidId('');setError('');if(pageIndex<pages.length-1){setPage(pageIndex+1);window.scrollTo({top:0});return;}setBusy(true);try{await onSubmit(wave,answers,startedAt);try{localStorage.removeItem(draft);}catch{}}catch(e){setError((e as Error).message);}finally{setBusy(false);}}
  return <section className="s1-card s1-form" aria-busy={busy}><p className="eyebrow">Step {pageIndex+1} of {pages.length}</p><h1 ref={heading} tabIndex={-1}>{wave==='pre'?'Before Big Brue':'After Big Brue'}</h1><p>{wave==='pre'?'Answer for yourself before attending any festival activities.':'Answer for yourself now that the festival is over.'}</p><progress className="s1-progress" value={pageIndex+1} max={pages.length} aria-label={`Questionnaire progress: step ${pageIndex+1} of ${pages.length}`}/><div className="s1-section-heading"><h2>{current.title}</h2><p>Choose a response for each question, or use “Prefer not to answer”.</p></div>{current.items.map(item=><Question invalid={invalidId===item.id} allowVoice={false} key={item.id} item={item} value={answers[item.id]} index={items.indexOf(item)} onChange={v=>{setAnswers(a=>({...a,[item.id]:v}));setError('');setInvalidId('');}}/>)}{error&&<p ref={errorSummary} tabIndex={-1} className="s1-error" role="alert">{error}{invalidId&&<> <a href={`#question-${invalidId}`} onClick={e=>{e.preventDefault();document.getElementById(`question-${invalidId}`)?.focus();}}>Go to question</a></>}</p>}<nav className="bf-nav" aria-label="Questionnaire navigation"><button className="secondary" disabled={pageIndex===0||busy} onClick={()=>{setPage(pageIndex-1);setError('');window.scrollTo({top:0});}}>Back</button>{!storageOK&&<span>Keep this page open; draft cannot be saved</span>}<button className="primary" disabled={busy||!loaded} onClick={next}>{busy?'Saving…':pageIndex<pages.length-1?'Continue':'Save my answers'}</button></nav></section>;
}
