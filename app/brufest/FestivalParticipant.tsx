...
import { CONTINUOUS_INSTRUMENT_VERSION, CONFLICT_PERCEPTION_INSTRUMENT_VERSION, isSelectiveInstrument, isContinuousInstrument } from '@/lib/brufest/festival/scales';
import { useEffect, useRef, useState } from 'react';
import Question from './Question';
import { answerError, questions, topicReferenceFromAnswers } from '@/lib/brufest/instruments';
...
export default function FestivalParticipant(){
  ...
  const [previewReference,setPreviewReference]=useState<TopicReference|undefined>();
  ...
  useEffect(()=>{
    ...
    if(params.get('preview')==='1'){
      ...
      if(['connection-close','connection-similar'].includes(params.get('step')??'')){
        const page=questions({study:'festival',role:'attendee',wave,festivalVersion:FESTIVAL_VERSION,responseInstrument:CONFLICT_PERCEPTION_INSTRUMENT_VERSION},{}).findIndex(q=>q.id==='E1_CONNECTION_SIMILAR');
  ...
  function connectionPage(wave:Wave){return Math.max(0,questions({study:'festival',role:'attendee',wave,festivalVersion:FESTIVAL_VERSION,responseInstrument:CONFLICT_PERCEPTION_INSTRUMENT_VERSION},{}).findIndex(q=>q.id==='E1_CONNECTION_SIMILAR'));}
  function previewView(wave:Wave):View{return {id:'preview-only',mode:'study',step:wave,message:'',contactChoiceSaved:false,permission:false,email:null,completed:[],responseInstrument:CONFLICT_PERCEPTION_INSTRUMENT_VERSION};}
  ...
  async function start(){setBusy(true);setError('');try{const v=await request({action:'enrol',access,agree,informationVersion:INFORMATION_VERSION,instrumentVersion:CONFLICT_PERCEPTION_INSTRUMENT_VERSION});setView(v);setRegistered(true);history.replaceState(null,'',`${STUDY_ONE_PUBLIC_PATH}#first=${access}`);try{localStorage.setItem(STORAGE,access);}catch{setStorageNote('Your browser cannot save progress. Keep this page open.');}window.scrollTo({top:0});}catch(e){setError((e as Error).message);}finally{setBusy(false);}}
...
function FestivalForm({access,wave,instrument,topicReference,initialPage,persistDraft=true,onSubmit}:{access:string;wave:Wave;instrument:string;topicReference?:TopicReference;initialPage?:number;persistDraft?:boolean;onSubmit:(wave:Wave,answers:Answers,startedAt:string)=>Promise<void>}){
  ...
  const items=questions({study:'festival',role:'attendee',wave,festivalVersion:FESTIVAL_VERSION,responseInstrument:instrument,topicReference},answers);
  const pages=items.map(q=>({title:q.section,items:[q]}));
  const pageIndex=Math.min(page,pages.length-1);
  const current=pages[pageIndex];
  const selectedTopic=wave==='post'?topicReference:topicReferenceFromAnswers(answers);
  const showTopicReference=current.items.some(q=>q.id==='E1_TOPIC_CERTAINTY'||q.id==='E1_TOPIC_RECONSIDER');
  ...
  return <section className="stage ratings-stage s1-card s1-form" aria-busy={busy}><div className="ratings-stage-deco" aria-hidden="true">
            ...
            />
          </div><p className="eyebrow">How We Disagree · {wave==='pre'?'Before Big Brue':'After Big Brue'}</p><div className="step-header s1-step-header"><h1 ref={heading} tabIndex={-1}>{current.title}</h1><p>{isSelectiveInstrument(instrument)&&pageIndex<(instrument===CONFLICT_PERCEPTION_INSTRUMENT_VERSION?18:16)?'Think about disagreements on issues that matter to you. Answer for how you see things now. '+(wave==='pre'?'Complete this before attending any festival activities.':'Answer now that the festival is over.'):(wave==='pre'?'Answer for yourself before attending any festival activities.':'Answer for yourself now that the festival is over.')}</p></div><div className="rating-focus-header"><span>Question {pageIndex+1} of {pages.length}</span><progress className="s1-progress" value={pageIndex+1} max={pages.length} aria-label={`Questionnaire progress: question ${pageIndex+1} of ${pages.length}`}/></div>{current.items.some(q=>q.id==='E1_PRE_STARTING_VIEW')&&typeof answers.E1_PRE_TOPIC_PANEL==='string'&&<p className="s1-question-help">{answers.E1_PRE_TOPIC_PANEL}</p>}{showTopicReference&&selectedTopic&&<div className="s1-information s1-topic-reference"><p>{wave==='pre'?'Thinking about your selected topic:':'Thinking about the same topic:'}</p><p><strong>{selectedTopic.topic}</strong></p><p>{wave==='pre'?'Your starting view:':'Before Big Brue, you wrote:'}</p><blockquote style={{whiteSpace:'pre-wrap',overflowWrap:'anywhere'}}>{selectedTopic.view}</blockquote><p>Answer about this same view as you see it now.</p></div>}{current.items.map(item=><Question motionDirection={direction} invalid={invalidId===item.id} allowVoice={false} key={item.id} item={item} value={answers[item.id]} index={items.indexOf(item)} onChange={v=>{setAnswers(a=>{const next={...a,[item.id]:v};if(item.id==='E1_PRE_TOPIC_PANEL'){delete next.E1_PRE_STARTING_VIEW;delete next.E1_TOPIC_CERTAINTY;delete next.E1_TOPIC_RECONSIDER;}if(item.id==='E1_PRE_STARTING_VIEW'){delete next.E1_TOPIC_CERTAINTY;delete next.E1_TOPIC_RECONSIDER;}return next;});setError('');setInvalidId('');}}/>)}{error&&<p ref={errorSummary} tabIndex={-1} className="s1-error" role="alert">{error}{invalidId&&<> <a href={`#question-${invalidId}`} onClick={e=>{e.preventDefault();document.getElementById(`question-${invalidId}`)?.focus();}}>Go to question</a></>}</p>}<nav className="actions bf-nav" aria-label="Questionnaire navigation"><button className="secondary" disabled={pageIndex===0||busy} onClick={()=>{setDirection('back');setPage(pageIndex-1);setError('');window.scrollTo({top:0});}}><ArrowLeft size={18} aria-hidden="true"/>Back</button>{!storageOK&&<span>Keep this page open; draft cannot be saved</span>}<button className="primary" disabled={busy||!loaded} onClick={next}>{busy?'Saving…':pageIndex<pages.length-1?'Continue':'Save my answers'}{!busy&&<ArrowRight size={18} aria-hidden="true"/>}</button></nav></section>;
}
