...
import { CONTINUOUS_INSTRUMENT_VERSION, SELECTIVE_INSTRUMENT_VERSION, isContinuousInstrument } from '@/lib/brufest/festival/scales';
...
export default function FestivalParticipant(){
  ...
  useEffect(()=>{
    ...
    if(params.get('preview')==='1'){
      ...
      if(['connection-close','connection-similar'].includes(params.get('step')??'')){
        const page=questions({study:'festival',role:'attendee',wave,festivalVersion:FESTIVAL_VERSION,responseInstrument:SELECTIVE_INSTRUMENT_VERSION},{}).findIndex(q=>q.id==='E1_CONNECTION_SIMILAR');
  ...
  function connectionPage(wave:Wave){return Math.max(0,questions({study:'festival',role:'attendee',wave,festivalVersion:FESTIVAL_VERSION,responseInstrument:SELECTIVE_INSTRUMENT_VERSION},{}).findIndex(q=>q.id==='E1_CONNECTION_SIMILAR'));}
  function previewView(wave:Wave):View{return {id:'preview-only',mode:'study',step:wave,message:'',contactChoiceSaved:false,permission:false,email:null,completed:[],responseInstrument:SELECTIVE_INSTRUMENT_VERSION};}
  ...
  async function start(){setBusy(true);setError('');try{const v=await request({action:'enrol',access,agree,informationVersion:INFORMATION_VERSION,instrumentVersion:SELECTIVE_INSTRUMENT_VERSION});setView(v);setRegistered(true);history.replaceState(null,'',`${STUDY_ONE_PUBLIC_PATH}#first=${access}`);try{localStorage.setItem(STORAGE,access);}catch{setStorageNote('Your browser cannot save progress. Keep this page open.');}window.scrollTo({top:0});}catch(e){setError((e as Error).message);}finally{setBusy(false);}}
