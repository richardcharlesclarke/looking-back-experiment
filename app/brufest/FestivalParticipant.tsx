...
import { CONTINUOUS_INSTRUMENT_VERSION } from '@/lib/brufest/festival/scales';
...
export default function FestivalParticipant(){
  ...
  return <main className="s1-participant"><header className="topbar"><a className="mark" href="https://experiments.evolvable.me">initiatives.evolvable.me</a><a href={EVOLVABLE_URL}>Explore evolvable.me</a></header><div className="s1-perspective-art" aria-hidden="true"><VectorDecoration src="/study-one/perspectives.svg" className="s1-perspectives" variant="profile-1" stroke="#F4F3F4" drawDuration="18s" activateImmediately/></div>
    ...
    </section>:view.step==='pre'||view.step==='post'?<FestivalForm key={`${access}:${view.step}`} access={access} wave={view.step} onSubmit={(wave,answers,startedAt)=>act({action:'submit',wave,answers,startedAt,instrumentVersion:CONTINUOUS_INSTRUMENT_VERSION})}/>:view.step==='contact'?<ContactForm onSubmit={(permission,email)=>act({action:'contact',permission,email})}/>:<section className="stage s1-card">
...
function FestivalForm({access,wave,onSubmit}:{access:string;wave:Wave;onSubmit:(wave:Wave,answers:Answers,startedAt:string)=>Promise<void>}){
  ...
  const draft=`study-one-draft:${CONTINUOUS_INSTRUMENT_VERSION}:${access}:${wave}`;
  const [legacyDraft,setLegacyDraft]=useState(false);
  const items=questions({study:'festival',role:'attendee',wave,festivalVersion:FESTIVAL_VERSION,responseInstrument:CONTINUOUS_INSTRUMENT_VERSION},answers);
  const pages=items.map(q=>({title:q.section,items:[q]}));
  const pageIndex=Math.min(page,pages.length-1);
  const current=pages[pageIndex];
  useEffect(()=>{try{const old=JSON.parse(localStorage.getItem(draft)||'null');setLegacyDraft(!old&&!!localStorage.getItem(`study-one-draft:${FESTIVAL_VERSION}:${access}:${wave}`));if(old?.answers&&typeof old.page==='number'){setAnswers(old.answers);setPage(old.page);setStartedAt(old.startedAt);}else setStartedAt(new Date().toISOString());}catch{setStartedAt(new Date().toISOString());setStorageOK(false);}setLoaded(true);},[draft,access,wave]);
  ...
  async function next(){let invalid=current.items.find(q=>answerError(q,answers[q.id]));if(!invalid&&pageIndex===pages.length-1)invalid=items.find(q=>answerError(q,answers[q.id]));if(invalid){setInvalidId(invalid.id);setError(`${invalid.prompt} ${answerError(invalid,answers[invalid.id])}`);setPage(pages.findIndex(p=>p.items.includes(invalid!)));return;}setInvalidId('');setError('');if(pageIndex<pages.length-1){setDirection('forward');setPage(pageIndex+1);window.scrollTo({top:0});return;}setBusy(true);try{await onSubmit(wave,answers,startedAt);try{localStorage.removeItem(draft);}catch{}}catch(e){setError((e as Error).message);}finally{setBusy(false);}}
  return <section className="stage s1-card s1-form" aria-busy={busy}><p className="eyebrow">How We Disagree · {wave==='pre'?'Before Big Brue':'After Big Brue'}</p><div className="step-header s1-step-header"><h1 ref={heading} tabIndex={-1}>{current.title}</h1><p>{wave==='pre'?'Answer for yourself before attending any festival activities.':'Answer for yourself now that the festival is over.'}</p></div><div className="rating-focus-header"><span>Question {pageIndex+1} of {pages.length}</span><progress className="s1-progress" value={pageIndex+1} max={pages.length} aria-label={`Questionnaire progress: question ${pageIndex+1} of ${pages.length}`}/></div>{legacyDraft&&<p className="s1-draft-note" role="status">The response scale has changed. Please answer again on the new scale. Your earlier draft has been kept in this browser.</p>}{current.items.map(item=><Question motionDirection={direction} invalid={invalidId===item.id} allowVoice={false} key={item.id} item={item} value={answers[item.id]} index={items.indexOf(item)} onChange={v=>{setAnswers(a=>({...a,[item.id]:v}));setError('');setInvalidId('');}}/>)}{error&&<p ref={errorSummary} tabIndex={-1} className="s1-error" role="alert">{error}{invalidId&&<> <a href={`#question-${invalidId}`} onClick={e=>{e.preventDefault();document.getElementById(`question-${invalidId}`)?.focus();}}>Go to question</a></>}</p>}<nav className="actions bf-nav" aria-label="Questionnaire navigation"><button className="secondary" disabled={pageIndex===0||busy} onClick={()=>{setDirection('back');setPage(pageIndex-1);setError('');window.scrollTo({top:0});}}><ArrowLeft size={18} aria-hidden="true"/>Back</button>{!storageOK&&<span>Keep this page open; draft cannot be saved</span>}<button className="primary" disabled={busy||!loaded} onClick={next}>{busy?'Saving…':pageIndex<pages.length-1?'Continue':'Save my answers'}{!busy&&<ArrowRight size={18} aria-hidden="true"/>}</button></nav></section>;
}
