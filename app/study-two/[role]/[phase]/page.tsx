...
import '../../review.css';
export async function generateMetadata({params}:{params:Promise<{role:string;phase:string}>}){const {role,phase}=await params;return {title:role==='speaker'?`Panel speakers · ${phase==='before'?'Before':'After'} the conversation`:`Study Two — ${role} ${phase} the panel`};}
