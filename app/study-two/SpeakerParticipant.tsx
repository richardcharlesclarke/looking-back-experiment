...
export default function SpeakerParticipant({wave}:{wave:Wave}){
 ...
 return <main className="s1-participant s2-speaker"><header className="topbar"><button type="button" className="s1-about-button" aria-haspopup="dialog" onClick={()=>studyInformation.current?.showModal()}>About this study</button></header>
  <dialog ref={studyInformation} className="s1-about-dialog" aria-labelledby="s2-about-title" onKeyDown={e=>{if(e.key==='Tab'){e.preventDefault();e.currentTarget.querySelector('button')?.focus();}}}><div className="s1-about-heading"><h2 id="s2-about-title">About this study</h2><button type="button" className="secondary" onClick={()=>studyInformation.current?.close()}>Close</button></div><section><h3>Study Two · Panel speakers</h3><p>Questions about your approach to conflict, the issue being discussed, yourself and the other panellists, before and after the conversation.</p></section><section><h3>Your responses</h3><p>You can decline any question. Progress saves to the Study Two server when you continue. Keep your private return link to reopen the same record and matched after questionnaire. </p></section></dialog>
