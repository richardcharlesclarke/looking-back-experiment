...
import {attemptOf,draftKey,obsoleteDraftKeys,pairing} from '@/lib/study-two/attempts';
...
function discardOldAttempts(access:string,run:SavedPerson){
 try{for(const wave of ['pre','post'] as const)for(const key of obsoleteDraftKeys(Object.keys(localStorage),run.instrumentVersion,access,wave,attemptOf(run,wave)))localStorage.removeItem(key);}catch{}
}
export default function SpeakerParticipant({wave}:{wave:Wave}){
 ...
 useEffect(()=>{let cancelled=false;async function load(){
  ...
  if(key){setAccess(key);try{
   ...
   if(!cancelled){discardOldAttempts(key,saved);setRun(saved);setAccess(key);history.replaceState(null,'',`${location.pathname}#access=${key}`);try{localStorage.setItem('study-two-return:speaker',key);}catch{}}
 ...
 const staleAfter=Boolean(run&&wave==='post'&&pairing(run).staleAfter);
 const form=run?.forms[wave],screen=!loaded?'loading':invalid?'invalid':run?.withdrawnAt?'withdrawn':beforeRequired?'before-required':needsInformation?'information':staleAfter?'earlier-after':!form?'intro':form.completedAt?'complete':'form';
 ...
 async function begin(){
  ...
  try{
   ...
   else if(run){saved=run;if(needsInformation)saved=await request({action:'consent',access:key,role,...consent});saved=await request({action:'start',access:key,role,wave,instrumentVersion:saved.instrumentVersion,attempt:attemptOf(saved,wave),beforeAttempt:attemptOf(saved,'pre')});}
   ...
   discardOldAttempts(key,saved);setAccess(key);setRun(saved);if(earlierRecord&&wave==='post'){location.assign(`/study-two/speaker/before#access=${key}`);return;}
   history.replaceState(null,'',`${location.pathname}#access=${key}`);window.scrollTo({top:0});
  }catch(e){setError((e as Error).message);if(key){const current=await request({action:'status',access:key,role}).catch(()=>null);if(current){discardOldAttempts(key,current);setRun(current);}}}finally{setBusy(false);}
 }
 function refreshed(current:SavedPerson){discardOldAttempts(access,current);setRun(current);setError('The organiser changed this questionnaire. The current attempt is shown below; earlier answers remain saved separately.');}
 async function withdraw(){setBusy(true);setError('');try{const saved=await request({action:'withdraw',access,role,confirm:true});setRun(saved);setWithdrawalOpen(false);try{if(localStorage.getItem('study-two-return:speaker')===access)localStorage.removeItem('study-two-return:speaker');for(const w of ['pre','post']){const prefix=`study-two-speaker-draft:${run?.instrumentVersion}:${access}:${w}`;for(const key of Object.keys(localStorage))if(key===prefix||key.startsWith(prefix+':'))localStorage.removeItem(key);}}catch{}}catch(e){setError((e as Error).message);}finally{setBusy(false);}}
...
function SpeakerForm({access,wave,run,onComplete,onRefresh}:{access:string;wave:Wave;run:SavedPerson;onRefresh:(run:SavedPerson)=>void;onComplete:(run:SavedPerson)=>void}){
 ...
 const attempt=attemptOf(run,wave),beforeAttempt=attemptOf(run,'pre');
 const draft=draftKey(run.instrumentVersion,access,wave,attempt);
 const pageIndex=Math.min(page,items.length-1),item=items[pageIndex];
 useEffect(()=>{try{const legacy=`study-two-speaker-draft:${run.instrumentVersion}:${access}:${wave}`;const saved=JSON.parse(localStorage.getItem(draft)||(attempt===1?localStorage.getItem(legacy):null)||'null');if(saved?.answers&&Number.isInteger(saved.page)&&saved.page>=0&&saved.page<items.length){if(saved.revision===initial.revision){setAnswers(saved.answers);setPage(saved.page);}else{localStorage.setItem(`${draft}:unmerged`,JSON.stringify(saved));}}}catch{setStorageOK(false);}setLoaded(true);},[draft,initial.revision,items.length,attempt,run.instrumentVersion,access,wave]);
 ...
 function save(nextAnswers:Record<string,Answer>,target:number,complete=false){
  const task=queue.current.then(async()=>{
   ...
   const payload={action:'save',access,role,wave,instrumentVersion:run.instrumentVersion,attempt,beforeAttempt,revision:serverRevision.current,answers:nextAnswers,page:target,complete};
   ...
   try{saved=await request(payload);}catch(e){
    ...
    if(current&&(current.withdrawnAt||attemptOf(current,wave)!==attempt||(wave==='post'&&(attemptOf(current,'pre')!==beforeAttempt||pairing(current).staleAfter)))){syncFailed.current=true;onRefresh(current);throw new Error('The questionnaire changed. Your earlier answers remain saved separately.');}
